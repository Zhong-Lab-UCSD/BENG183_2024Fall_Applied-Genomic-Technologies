...
#### Motif Discovery Mode Applications
MEME can be utilized through any of the three modes mentioned previously. A couple of possible application instances include using Classic Mode to search for transcription factor binding site motifs in a set of DNA sequences, and Differential or Discriminative Mode could be used interchangeably to compare protein expression levels between healthy and diseased cells.

## 1.1.5 Limitations<a name="115"></a>

One important limitation to note is that MEME can only handle ungapped motifs (fixed-length patterns). Therefore, variable-length patterns must be split into multiple separate “ungapped” motifs. For datasets with gapped motifs, it is recommended to use GLAM2 instead of MEME from MEME Suite, which is specialized for such calculations. 
